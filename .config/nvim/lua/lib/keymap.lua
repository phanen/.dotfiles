...
---@param opts table?
M.cabbr = function(trig, command, opts)
  ...
  map(
    'ca',
    trig,
    function() return fn.getcmdcompltype() == 'command' and command or trig end,
...
---@param command string
---@param opts table?
M.cmap = function(trig, command, opts)
  map(
    'c',
    trig,
    function() return fn.getcmdcompltype() == 'command' and command or trig end,
...
M.fallback_fn = function(def)
  ...
  local function feed(keys)
    ...
    api.nvim_feedkeys(keyseq, modes, false)
  end
  ...
  if def.callback then
    ...
    -- special characters
    local rhs = fn.escape(def.rhs, '\\')
    return function() feed(api.nvim_eval(rhs)) end
  end
end
...
---@return fun(): T[]
M.count_wrap = function(fn, count)
  return function()
    if count == 0 and vim.v.count == 0 then return {} end
    local result = {}
