...
local function save_win_state(states, winnr)
  ...
  for _, option in ipairs(tracked_win_options) do
    win_options[option] = vim.wo[winnr][option]
  end
  states[winnr] = {
...
local function set_win_options(winnr, options)
  log('set_win_options(): winnr=', winnr, ', options=', options)
...
local function peek(winnr, linenr)
  log(('peek(), winnr=%d, linenr=%d'):format(winnr, linenr))
  ...
  if opts.hide_relativenumbers then peeking_options.relativenumber = false end
  ...
  if opts.centered_peeking then cmd 'normal! zz' end
end
...
function Numb.on_cmdline_exit()
  ...
  if not is_peeking(winnr) then
    log(winnr .. ' is not at peek state, returning')
    return
  end
