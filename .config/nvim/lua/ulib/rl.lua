...
Rl.alphanum = alphanum
Rl.default_word_chars = alphanum
Rl.word_chars = {}

local function is_whitespace(c) return c == ' ' or c == '	' end
...
local function command_line_mode() return vim.fn.mode() == 'c' end
...
local function block_cursor_mode() return BLOCK_CURSOR_MODES[vim.fn.mode()] end
...
local function get_line(line_no) return vim.fn.getline(line_no) end

local function get_char(s, char_idx) return vim.fn.nr2char(vim.fn.strgetchar(s, char_idx)) end
...
local function curr_cursor_col()
  ...
  if command_line_mode() then
    ...
    if byte_idx == vim.fn.strlen(line) then return vim.fn.strchars(line) end
...
local function cursor_col_at_end_of_leading_whitespace(line)
  local char_idx = 0
  local function char() return get_char(line, char_idx) end
...
local function cursor_col_at_start_of_trailing_whitespace(line)
  ...
  local function prev_char() return get_char(line, char_idx - 1) end
...
local function get_word_chars()
  if command_line_mode() then
    return Rl.default_word_chars -- Hmm, we can probably do better than this.
  end

  return vim.b.readline_word_chars or Rl.word_chars[vim.bo.filetype] or Rl.default_word_chars
end
...
local function get_stop_patterns() return STOP_PATTERNS[vim.o.ft] or {} end
...
local function new_cursor(s, char_idx, dir, word_chars)
  ...
  local next_char_idx = function(j) return (dir == -1) and (j - 1) or j end
  ...
  while can_advance(char_idx) do
    ...
    if
      (is_whitespace(c) and consumed_non_whitespace)
      or (not is_word_char(c, word_chars) and consumed_word_char)
    then
      break
    end
    ...
    if char_idx == early_exit then return char_idx end
  end
  return char_idx
end
...
local function build_trie(ss)
  ...
  for _, s in ipairs(ss) do
    ...
    for byte_idx = 1, #s do
      ...
      if not node.children[c] then node.children[c] = build_trie_node() end
...
local function backward_line_stops(s, stop_patterns)
  ...
  if first_stop > result[#result] then table.insert(result, first_stop) end
  ...
  if hit then
    ...
    if char_idx > result[#result] then table.insert(result, char_idx) end
  end

  return result
end

function Rl._run_tests()
  local messages = {}
...
local function yank_to_small_delete_register(cursor_col_1, cursor_col_2)
  if cursor_col_1 == cursor_col_2 then return end
...
local function kill_to(end_line_no, end_cursor_col)
  ...
  if end_line_no == start_line_no and end_cursor_col == start_cursor_col then return end
  ...
  if command_line_mode() then
    ...
    local ltr = start_line_no < end_line_no
      or (start_line_no == end_line_no and start_cursor_col < end_cursor_col)
...
function Rl.forward_word() move_cursor_to(forward_word_location(get_word_chars())) end

function Rl.backward_word() move_cursor_to(backward_word_location(get_word_chars())) end

function Rl.end_of_line() move_cursor_to(curr_line_no(), last_cursor_col_on_curr_line()) end

function Rl.beginning_of_line() move_cursor_to(curr_line_no(), 0) end

function Rl.dwim_beginning_of_line() move_cursor_to(dwim_beginning_of_line_pos()) end

function Rl.back_to_indentation()
  move_cursor_to(curr_line_no(), cursor_col_at_end_of_leading_whitespace(curr_line()))
end

function Rl.kill_word() kill_to(forward_word_location(get_word_chars())) end

function Rl.backward_kill_word() kill_to(backward_word_location(get_word_chars())) end

function Rl.unix_word_rubout() kill_to(backward_word_location('NON_WHITESPACE_CHARS')) end

function Rl.kill_line() kill_to(curr_line_no(), last_cursor_col_on_curr_line()) end

function Rl.backward_kill_line() kill_to(curr_line_no(), 0) end
