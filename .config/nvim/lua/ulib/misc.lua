...
Misc.quit = function()
  ...
  if quit_fts[vim.bo.ft] then return vim.schedule(function() vim.cmd('quit!') end) end
  return api.nvim_feedkeys('q', 'n', false)
end
...
Misc.tabnext_or_tabnew = function()
  if fn.tabpagenr('$') == 1 then
    local name = fn.bufname('#')
    if #name ~= 0 then return vim.cmd.tabnew(name) end
    return vim.cmd.tabnew(name)
  end
...
---@cmd
Misc.keywordprg = function(opts) -- fallback: lsp -> manpage
  local bufnr = api.nvim_get_current_buf()
  if u.is.has_lsp(bufnr) then return vim.lsp.buf.hover() end
  local man = require('man')
  local ok, err = pcall(man.open_page, opts.count, opts.smods, opts.fargs)
  if not ok then vim.notify(man.errormsg or err, vim.log.levels.ERROR) end
end
...
Misc.archive = function()
  local archive_dir = env.HOME .. '/b/extra'
  local archive = fs.joinpath(archive_dir, fn.expand('%:t'))
  local mode = api.nvim_get_mode().mode
  local lines = u.buf.getregion(mode)
  local curr = fn.expand('%:p')
  vim.ui.input({ prompt = 'Archive to:', default = archive }, function(input)
    archive = vim.trim(input)
    if mode:match('[vV\022]') then
      u.fs.must_writefile(lines, archive, 'a')
    else
      u.fs.must_filecopy(curr, archive)
    end
    vim.cmd.vsplit(archive)
    -- u.git { cwd = archive_dir }.commit("update " .. archive_file)
    -- u.git { cwd = archive_dir }.push()
  end)
end
...
Misc.lazy_patch_autocmd = function(ev)
  ...
  u.script.update_lazy_patch(should_patch)
  u.script.update_lazy_cache_docs()
end
...
---@autocmd
Misc.lsp_attach = function(_)
  vim.lsp.handlers['textDocument/signatureHelp'] =
    vim.lsp.with(vim.lsp.handlers.signature_help, { border = g.border })
  vim.lsp.handlers['textDocument/hover'] =
    vim.lsp.with(vim.lsp.handlers.hover, { border = g.border })
  vim.diagnostic.config {
    float = { border = g.border },
  }
end
