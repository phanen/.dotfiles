...
Script.update_meta = function(path)
  ...
  u.fs.must_writefile(lines, path)
  u.fmt.conform { buf = fn.bufadd(path) }
  g.updating_meta = true
end
...
Script.update_spec = function(path)
  ...
  u.fmt.conform { async = true, buf = buf }

  g.updating_specs = false
end

Script.update_chore = function()
  Script.update_lazy_patch(true)
