...
local utils = require('mod.winbar.utils')
local groupid = ag('WinBarMenu', {})
  ...
  local entry = setmetatable(
    vim.tbl_deep_extend('force', {
      separator = bar.winbar_symbol_t:new {
        icon = config.opts.icons.ui.menu.separator,
        icon_hl = 'WinBarIconUISeparatorMenu',
      },
      padding = config.opts.menu.entry.padding,
      components = {},
    }, opts or {}),
    self
  )
...
function winbar_menu_entry_t:cat()
  ...
  for component_idx, component in ipairs(self.components) do
    if component_idx > 1 then table.insert(components_with_sep, self.separator) end
...
---@return number
function winbar_menu_entry_t:displaywidth() return fn.strdisplaywidth((self:cat())) end

---Get the byte length of the winbar menu entry
---@return number
function winbar_menu_entry_t:bytewidth() return #(self:cat()) end
...
function winbar_menu_t:new(opts)
  local winbar_menu = setmetatable(
    vim.tbl_deep_extend('force', {
      entries = {},
      win_configs = config.opts.menu.win_configs,
    }, opts or {}),
    winbar_menu_t
  )
...
function winbar_menu_t:del()
  ...
  self:close()
  if self.buf then
    if api.nvim_buf_is_valid(self.buf) then api.nvim_buf_delete(self.buf, {}) end
    self.buf = nil
  end
  if self.win then _G._winbar.menus[self.win] = nil end
end
...
function winbar_menu_t:eval_win_configs()
  ...
  if self._win_configs.relative ~= 'win' then self._win_configs.win = nil end
end
...
function winbar_menu_t:get_component_at(pos, look_ahead)
  if not self.entries or vim.tbl_isempty(self.entries) then return nil, nil end
  local entry = self.entries[pos[1]]
  if not entry or not entry.components then return nil, nil end
...
function winbar_menu_t:click_on(symbol, min_width, n_clicks, button, modifiers)
  ...
  for idx, component in ipairs(symbol.entry.components) do
    if idx == symbol.entry_idx then break end
    col = col + component:bytewidth() + symbol.entry.separator:bytewidth()
  end
  self.clicked_at = { row, col }
  if symbol and symbol.on_click then symbol:on_click(min_width, n_clicks, button, modifiers) end
end
...
function winbar_menu_t:update_current_context_hl(linenr)
  if self.buf then utils.hl.line_single(self.buf, 'WinBarMenuCurrentContext', linenr) end
end
...
function winbar_menu_t:make_buf()
  ...
  for _, entry in ipairs(self.entries) do
    ...
    -- the entire line
    table.insert(lines, line .. string.rep(' ', self._win_configs.width - fn.strdisplaywidth(line)))
  ...
  if self.cursor then self:update_current_context_hl(self.cursor[1]) end
  ...
  autocmd('CursorMoved', {
    ...
    callback = function()
      ...
      if config.opts.menu.preview then
        self:preview_symbol_at(cursor, true)
      else
        self.prev_cursor = cursor
      end

      if config.opts.menu.quick_navigation then self:quick_navigation(cursor) end
...
function winbar_menu_t:open_win()
  ...
  self.win = api.nvim_open_win(self.buf, true, self._win_configs)
...
function winbar_menu_t:update_scrollbar()
  if
    ...
    or not config.opts.menu.scrollbar.enable
  then
    return
  end

  local buf_height = api.nvim_buf_line_count(self.buf)
  local menu_win_configs = api.nvim_win_get_config(self.win)
  if buf_height <= menu_win_configs.height then
    self:close_scrollbar()
    return
  end

  local thumb_height = math.max(1, math.floor(menu_win_configs.height ^ 2 / buf_height))
  local offset = fn.line('w$') == buf_height and menu_win_configs.height - thumb_height
    or math.min(
      menu_win_configs.height - thumb_height,
      math.floor(menu_win_configs.height * fn.line('w0') / buf_height)
    )

  if self.scrollbar and api.nvim_win_is_valid(self.scrollbar.thumb) then
    local config = api.nvim_win_get_config(self.scrollbar.thumb)
    config.row = offset
    config.height = thumb_height
    api.nvim_win_set_config(self.scrollbar.thumb, config)
  else
    self:close_scrollbar()
    self.scrollbar = {}
    local win_configs = {
    ...
    self.scrollbar.background =
      api.nvim_open_win(api.nvim_create_buf(false, true), false, win_configs)
    ...
    self.scrollbar.thumb = api.nvim_open_win(api.nvim_create_buf(false, true), false, win_configs)
...
function winbar_menu_t:close_scrollbar()
  ...
  if api.nvim_win_is_valid(self.scrollbar.thumb) then
    api.nvim_win_close(self.scrollbar.thumb, true)
  end
  if api.nvim_win_is_valid(self.scrollbar.background) then
    api.nvim_win_close(self.scrollbar.background, true)
  end
  self.scrollbar = nil
end
...
function winbar_menu_t:open(opts)
  ...
  if self.prev_menu then
    ...
    if self.prev_menu.sub_menu then self.prev_menu.sub_menu:close() end
  ...
  if self._win_configs.focusable ~= false then
    if self.prev_cursor then
      api.nvim_win_set_cursor(self.win, self.prev_cursor)
    elseif self.cursor then
      api.nvim_win_set_cursor(self.win, self.cursor)
      api.nvim_exec_autocmds('CursorMoved', { buffer = self.buf })
    end
  end
  self:update_scrollbar()
end
...
function winbar_menu_t:close(restore_view)
  ...
  -- Close sub-menus
  if self.sub_menu then self.sub_menu:close(restore_view) end
  -- Move cursor to the previous window
  if self.prev_win and api.nvim_win_is_valid(self.prev_win) then
    api.nvim_set_current_win(self.prev_win)
  end
  -- Close the menu window and dereference it in the lookup table
  if self.win then
    if api.nvim_win_is_valid(self.win) then api.nvim_win_close(self.win, true) end
    _G._winbar.menus[self.win] = nil
    self.win = nil
  end
  if self.scrollbar then self:close_scrollbar() end
  -- Finish preview
  if config.opts.menu.preview then self:finish_preview(restore_view) end
...
function winbar_menu_t:finish_preview(restore_view)
  ...
  if self.symbol_previewed then
    ...
    if restore_view then self.symbol_previewed:preview_restore_view() end
...
function winbar_menu_t:quick_navigation(new_cursor)
  ...
  if target_component and range then
    ...
    api.nvim_win_set_cursor(self.win, new_cursor)
  end
