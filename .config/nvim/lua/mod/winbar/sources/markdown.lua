...
local groupid = ag('WinBarMarkdown', {})
...
local function parse_buf(buf, lnum_end, incremental)
  ...
  local lines = api.nvim_buf_get_lines(buf, lnum_start, lnum_end, false)
...
local function get_symbols(buf, win, cursor)
  ...
  if buf_symbols['end'].lnum < cursor[1] then
    parse_buf(
      buf,
      cursor[1] + config.opts.sources.markdown.parse.look_ahead,
      true
    )
  end
  local result = {}
