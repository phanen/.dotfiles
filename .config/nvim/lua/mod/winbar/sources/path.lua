...
local function convert(path, buf, win)
  ...
  return bar.winbar_symbol_t:new(setmetatable({
    ...
    jump = function(_) vim.cmd.edit(path) end,
  }, {
    ...
    __index = function(self, k)
      if k == 'children' then
        ...
        for name in vim.fs.dir(path) do
          if path_opts.filter(name) then
            table.insert(self.children, convert(vim.fs.joinpath(path, name), buf, win))
          end
        end
      ...
      if k == 'siblings' or k == 'sibling_idx' then
        ...
        for idx, name in vim.iter(vim.fs.dir(parent_dir)):enumerate() do
          if path_opts.filter(name) then
            table.insert(self.siblings, convert(vim.fs.joinpath(parent_dir, name), buf, win))
