...
if not fzf_lib then return false end
...
---@field menu_entries winbar_menu_entry_t[]
...
function fzf_state_t:new(menu, win, opts)
  ...
  for i = 1, num_entries do
    ...
    while c and c <= line_size do
      ...
      if retain_inner_spaces and not encountered_space and line:byte(c) == space_char then
  ...
  getmetatable(state.proxy).__gc = function(_) state:gc() end
  return state
end
...
function fzf_state_t:gc()
  if self.slab ~= nil then fzf_lib.free_slab(self.slab) end
  self.slab = nil
end
