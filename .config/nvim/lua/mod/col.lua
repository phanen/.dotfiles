...
local function get_hl_hex(hlgroup_name, field, fallback)
  ...
  if not fn.hlexists(hlgroup_name) then return fallback end
  ...
  -- of `utils.hl.get()`
  local attr_val = u.hl.get(0, { name = hlgroup_name, winhl_link = false })[field]
  return attr_val and u.hl.dec2hex(attr_val, 6) or fallback
end
...
local function cc_conceal(winid)
  api.nvim_win_call(winid or 0, function() vim.opt_local.winhl:append({ ColorColumn = '' }) end)
end
...
local function cc_show(winid)
  api.nvim_win_call(
    winid or 0,
    function() vim.opt_local.winhl:append({ ColorColumn = '_ColorColumn' }) end
  )
end
...
local function cc_update(winid)
  ...
  local length = fn.strdisplaywidth(fn.getline('.'))
  ...
  if vim.go.termguicolors then
    if not C_CC or not C_NORMAL or not C_ERROR then update_hl_hex() end
    local new_cc_color = show_warning and u.hl.cblend(C_ERROR, C_NORMAL, 0.4).dec
      or u.hl.cblend(C_CC, C_NORMAL, (length - thresh) / (cc - thresh)).dec
    if new_cc_color ~= cc_bg then
      cc_bg = new_cc_color
      api.nvim_set_hl(0, '_ColorColumn', { bg = cc_bg })
    end
  else
    local link = show_warning and 'Error' or 'ColorColumn'
    if cc_link ~= link then
      cc_link = link
      api.nvim_set_hl(0, '_ColorColumn', { link = cc_link })
    end
  end
  cc_show(winid)
end
...
autocmd({ 'BufWinEnter', 'ColorScheme' }, {
  ...
  group = id,
  callback = function()
    if fn.mode():find('^[icRss\x13]') then
      cc_update(0)
    else
      cc_conceal(0)
    end
...
autocmd({ 'CursorMovedI', 'InsertEnter' }, {
  ...
  group = id,
  callback = function() cc_update(0) end,
})
