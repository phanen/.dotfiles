...
local function get_query_strings_from_regex(query_strings_regex, query_group, lang)
  ...
  for _, regex in ipairs(query_strings_regex) do
    for _, capture in ipairs(available_textobject_captures) do
      if string.match('@' .. capture, regex) then table.insert(query_strings, '@' .. capture) end
    end
  end
...
local function ts_obj_move(opts)
  ...
  query_strings_regex = type(query_strings_regex) == 'string' and { query_strings_regex }
    or query_strings_regex
  local starts
  ...
  for _ = 1, vim.v.count1 do
    ...
    for _, query_string in ipairs(query_strings) do
      for _, start_ in ipairs(starts) do
        ...
        local current_match = queries.find_best_match(
          bufnr,
          query_string,
          query_group,
          function(match) return filter_function(start_, match) end,
          function(match) return scoring_function(start_, match) end
        )
