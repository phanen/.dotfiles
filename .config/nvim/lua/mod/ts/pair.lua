...
local set_pair_move_func = function(opts)
  ...
  vim.notify('Move mode: Use j or k to move, any other char to abort: ', vim.log.levels.INFO)
  while true do
    vim.cmd.normal('zz')
    vim.cmd.redraw()
    ...
    if not jump_ok then vim.notify(err, vim.log.levels.WARN) end
  end
...
M.setkeymap = function(key, opts)
  vim.keymap.set('n', '<leader>]' .. key, function() set_pair_move_func(opts) end)
end
