...
local supports_method = function(method, bufnr)
...
local setup_keymaps = function()
  ...
  ---@return function
  local goto_diagnostic = function(direction, level)
    return function()
  ...
  ---@return function
  local if_supports_lsp_method = function(method, fn_name, fallback)
    return function()
      return supports_method(method, 0)
          and string.format('<Cmd>lua vim.lsp.buf["%s"]()<CR>', fn.escape(fn_name, '"\\'))
        or fallback
    end
  end
  -- stylua: ignore start
  ...
  vim.keymap.set({ 'n', 'x' }, '[e', u.keymap.count_wrap(vim.diagnostic.goto_prev))
  vim.keymap.set({ 'n', 'x' }, ']e', u.keymap.count_wrap(vim.diagnostic.goto_next))
  vim.keymap.set({ 'n', 'x' }, '[E', u.keymap.count_wrap(goto_diagnostic('prev', 'ERROR')))
  vim.keymap.set({ 'n', 'x' }, ']E', u.keymap.count_wrap(goto_diagnostic('next', 'ERROR')))
  vim.keymap.set({ 'n', 'x' }, '[W', u.keymap.count_wrap(goto_diagnostic('prev', 'WARN')))
  vim.keymap.set({ 'n', 'x' }, ']W', u.keymap.count_wrap(goto_diagnostic('next', 'WARN')))
  vim.keymap.set({ 'n', 'x' }, '[I', u.keymap.count_wrap(goto_diagnostic('prev', 'INFO')))
  vim.keymap.set({ 'n', 'x' }, ']I', u.keymap.count_wrap(goto_diagnostic('next', 'INFO')))
  vim.keymap.set({ 'n', 'x' }, '[H', u.keymap.count_wrap(goto_diagnostic('prev', 'HINT')))
  vim.keymap.set({ 'n', 'x' }, ']H', u.keymap.count_wrap(goto_diagnostic('next', 'HINT')))
...
---@return nil
local setup_lsp_overrides = function()
  ...
  au('VimResized', {
    ...
    group = ag('LspUpdateFloatingWinMaxSize', {}),
    callback = function()
  ...
  function vim.lsp.util.open_floating_preview(contents, syntax, opts)
    local source_ft = vim.bo[api.nvim_get_current_buf()].ft
...
local parse_cmdline_args = function(fargs, fn_name_alt)
  local fn_name = fn_name_alt or fargs[1] and table.remove(fargs, 1) or nil
  local parsed = u.command.parse_cmdline_args(fargs)
...
local subcommand_arg_handler = {
  ...
  item = function(args)
    for _, item in pairs(args) do
      return type(item) == 'string' and uv.fs_realpath(item) or item
    end
  end,
...
local subcommand_completion = {
  bufs = function() return api.nvim_list_bufs() end,
  ---Get completion for LSP clients
  ---@return string[]
  lsp_clients = function(arglead)
    if arglead ~= '' then return {} end
    return vim.tbl_map(
      function(client) return string.format('%d (%s)', client.id, client.name) end,
      vim.lsp.get_clients()
    )
  end,
  ...
  lsp_client_ids = function(arglead)
    if arglead ~= '' then return {} end
    return vim.tbl_map(function(client) return client.id end, vim.lsp.get_clients())
  end,
...
local subcommands = {
  ...
  lsp = {
    info = {
      ...
      arg_handler = function(args) return args.filter end,
    ...
    restart = {
      ...
      fn_override = function(ids)
        ...
        local clients = not vim.tbl_isempty(ids)
            and vim.tbl_map(function(id) return vim.lsp.get_client_by_id(id) end, ids)
          or vim.lsp.get_clients({ bufnr = 0 })
        for _, client in ipairs(clients) do
          u.lsp.restart(client)
    ...
    get_clients_by_id = {
      completion = subcommand_completion.lsp_clients,
      arg_handler = function(args) return tonumber(args[1]:match('^%d+')) end,
      fn_override = function(id) vim.notify(vim.inspect(vim.lsp.get_client_by_id(id))) end,
    },
    get_clients = {
      opts = {
        'filter',
    ...
    auto_format = {
      ...
      fn_override = function(args, tbl)
        local enabled = u.classes.bufopt_t:new('lsp_autofmt_enabled') ---@type bufopt_t
        local fmtopts = u.classes.bufopt_t:new('lsp_autofmt_opts') ---@type bufopt_t
    ...
    add_workspace_folder = {
      ...
      completion = function(arglead, _, _)
        local basedir = arglead == '' and fn.getcwd() or arglead
        local incomplete = nil ---@type string|nil
        if not uv.fs_stat(basedir) then
          basedir = fn.fnamemodify(basedir, ':h')
          incomplete = fn.fnamemodify(arglead, ':t')
        end
        local subdirs = {}
        for name, type in vim.fs.dir(basedir) do
          if type == 'directory' and name ~= '.' and name ~= '..' then
            table.insert(
              subdirs,
              fn.fnamemodify(fn.resolve(vim.fs.joinpath(basedir, name)), ':p:~:.')
            )
          end
        end
        if incomplete then
          return vim.tbl_filter(function(s) return s:find(incomplete, 1, true) end, subdirs)
        end
        return subdirs
      end,
    },
    remove_workspace_folder = {
      ...
      completion = function(_, _, _)
        return vim.tbl_map(function(path)
          local short = fn.fnamemodify(path, ':p:~:.')
  ...
  diagnostic = {
    ...
    is_enabled = {
      ...
      fn_override = function(...) vim.notify(vim.inspect(vim.diagnostic.is_enabled(...))) end,
    },
    match = {
      ---@param args lsp_command_parsed_arg_t
    ...
    toqflist = {
      arg_handler = subcommand_arg_handler.item,
      opts = { 'diagnostics' },
      fn_override = function(...) fn.setqflist(vim.diagnostic.toqflist(...)) end,
    },
  },
}
...
local command_meta = function(subcommand_info_list, fn_scope, fn_name_alt)
...
local command_complete = function(meta, subcommand_info_list)
  ...
  return function(arglead, cmdline, cursorpos)
    ...
    local subcommand = u.string.camel_to_snake(cmdline:match('^%s*' .. meta .. '(%w+)'))
    ...
    if subcommand_info then
      return u.command.complete(subcommand_info.params, subcommand_info.opts)(
        arglead,
        cmdline,
        cursorpos
      )
...
---@param fn_scope table|fun(name: string): function scope of corresponding functions for subcommands
---@return nil
local setup_commands = function(meta, subcommand_info_list, fn_scope)
  -- metacommand -> MetaCommand abbreviation
  u.command_abbrev(meta:lower(), meta)
  -- Format: MetaCommand sub_command opts ...
  api.nvim_create_user_command(meta, command_meta(subcommand_info_list, fn_scope), {
    bang = true,
    range = true,
    nargs = '*',
  ...
  for subcommand, _ in pairs(subcommand_info_list) do
    api.nvim_create_user_command(
      meta .. u.string.snake_to_camel(subcommand),
...
---@return nil
local setup_lsp_autoformat = function()
  ---@type bufopt_t
  local enabled = u.classes.bufopt_t:new('lsp_autofmt_enabled', false)
  ---@type bufopt_t
  local fmtopts = u.classes.bufopt_t:new('lsp_autofmt_opts', {
    async = true,
    timeout_ms = 500,
  })
  au('BufWritePre', {
    ...
    callback = function(info)
      if enabled:get(info.buf) then vim.lsp.buf.format(fmtopts:get(info.buf)) end
    end,
  })
end
...
---
---@return nil
local setup_lsp_stopidle = function()
  au('BufDelete', {
    group = ag('LspAutoStop', {}),
    ...
    callback = function()
      ...
      vim.defer_fn(function()
        ...
        for _, client in ipairs(vim.lsp.get_clients()) do
          if vim.tbl_isempty(client.attached_buffers) then u.lsp.soft_stop(client) end
        end
      end, 60000)
    end,
  })
...
local setup_diagnostic = function()
  ...
  vim.diagnostic.config({
    virtual_text = {
      spacing = 4,
      prefix = vim.trim(u.static.icons.AngleLeft),
    },
    signs = {
      text = {
...
---@return nil
M.setup = function()
  if vim.g.loaded_lsp_diags ~= nil then return end
  ...
  setup_commands(
    'Lsp',
    subcommands.lsp,
    function(name) return vim.lsp[name] or vim.lsp.buf[name] end
  )
