...
---@return nil
M.setup = function()
  if vim.g.loaded_im ~= nil then return end
  vim.g.loaded_im = true
  if
    env.SSH_TTY
    or uv.os_uname().sysname:match('nix$') and not env.DISPLAY and not env.WAYLAND_DISPLAY
  then
    return
  end
  ...
  local function inside_input_mode()
    local mode = fn.mode()
    return (mode:find('^[itRss\x13]') or mode:find('^c') and fn.getcmdtype():find('[/?@-]'))
        and true
      or false
  end
    ...
    vim.system({ fcitx_cmd }, {}, function(obj)
      if obj.code ~= 0 or tonumber(obj.stdout) == 2 then
        ...
        vim.schedule(function()
          if api.nvim_buf_is_valid(vim.g.__im_input_enter) then
