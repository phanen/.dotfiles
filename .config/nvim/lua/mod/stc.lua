...
local function make_hl(str, hl, restore) return u.stl.hl(str, hl, restore, true) end
  ...
  do
    ...
    for _, sign in ipairs(data.extsigns) do
      ...
      if
        ...
        and (not sign_details or current_sign_details.priority > sign_details.priority)
      then
    ...
    if sign_details then
      return make_hl(
        ...
        data.culhl and sign_details.cursorline_hl_group or sign_details.sign_hl_group
      )
    end
  end
  ::signcol_ret_default::
...
function builders.lnum(data)
  ...
  if data.relnum == 0 then
    return string.format('%%=%-' .. math.max(data.nuw - 1, data.lnumw or 0) .. 'd ', data.lnum)
  end
  result = data.relnum

  ::lnum_ret_default::
  return string.format('%%=%' .. math.max(data.nuw - 1, data.lnumw or 0) .. 's ', result)
end
...
---@param sign extmark_sign_t
---@return boolean
local function nongitsigns_filter(sign) return not extsign_get_name(sign):find('^Git') end
...
function _G._stc()
  ...
  if not data.display_tick or data.display_tick < display_tick then -- Update shared data
    ...
    data.extsigns = api.nvim_buf_get_extmarks(
      buf,
      -1,
      { wininfo.topline - 1, 0 },
      { wininfo.botline - 1, -1 },
    ...
    if data.nu and data.rnu then
      local buf_tick = api.nvim_buf_get_changedtick(buf)
      if not data.buf_tick or data.buf_tick < buf_tick then
        lnumw_cache[buf] = numdigits(api.nvim_buf_line_count(buf))
...
local setup = function()
  if vim.g.loaded_statuscolumn ~= nil then return end
  ...
  augroup('StatusColumn', {
    ...
    'WinClosed',
    {
      ...
      callback = function(ev) shared[tonumber(ev.match)] = nil end,
    },
  }, {
    'BufDelete',
    {
      desc = 'Clear per buffer lnum width cache',
      callback = function(ev) lnumw_cache[ev.buf] = nil end,
    },
  })
end
