...
M.opts = {
  ...
  renderer = function(n, base, winnr, line)
    ...
    if not extmark_ns then extmark_ns = vim.api.nvim_create_namespace('Based') end
...
setmetatable(M.opts.patterns, {
  __index = function(table, _) return table.c end,
})
...
local buf_parse_int = function(str, base, bufnr)
  ...
  if base == 'hex' then
    ...
    if not n then n = parse_int(str, 16, M.opts.fallback_patterns.hex) end
  elseif base == 'dec' then
    n = parse_int(str, 10, patterns.dec)
    if not n then n = parse_int(str, 16, M.opts.fallback_patterns.dec) end
  else
    n = parse_int(str, 16, patterns.hex)
    base = 'hex'
    if not n then
...
local parse_and_render = function(str, base, winnr, line)
  ...
  if n then M.opts.renderer(n, found_base, winnr, line) end
end
...
local visual = function(base)
  ...
  if vend[2] - vstart[2] ~= 0 then return end
...
vim.api.nvim_create_user_command(
  'BasedConvert',
  function(opts) M.convert(opts.fargs[1]) end,
  { nargs = '?' }
)
...
M.setup = function(opts) M.opts = vim.tbl_deep_extend('force', M.opts, opts) end
