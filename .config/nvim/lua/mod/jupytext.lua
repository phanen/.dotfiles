...
---@param buf integer?
---@return nil
local jupytext_convert = function(buf)
  buf = buf ~= 0 and buf or api.nvim_get_current_buf()
  if not api.nvim_buf_is_valid(buf) then return end

  local fpath_cache = fn.stdpath('cache') --[[@as string]]
  local fpath_ipynb = vim.fs.normalize(api.nvim_buf_get_name(buf))
  local fpath_cache_jupytext = vim.fs.joinpath(fpath_cache, 'jupytext')
  if
    not uv.fs_stat(fpath_cache_jupytext) and not uv.fs_mkdir(fpath_cache_jupytext, 511)
  then
  ...
  local fpath_out =
    vim.fs.joinpath(fpath_cache_jupytext, (fn.fnamemodify(fpath_ipynb, ':r'):gsub('/', '%%')))
  local fpath_md = fpath_out .. '.md'
  local fpath_sha = fpath_out .. '.sha256'
  local prev_sha, cur_sha
  if fn.executable('sha256sum') == 1 then
    if uv.fs_stat(fpath_sha) then
    ...
    vim
      .system({
        'sha256sum',
        fpath_ipynb,
      }, {}, function(obj) cur_sha = vim.trim(obj.stdout) end)
      :wait()
  end
  ...
  if uv.fs_stat(fpath_ipynb) then
    ...
    if prev_sha ~= cur_sha or not prev_sha or not cur_sha or not uv.fs_stat(fpath_md) then
      vim
        ...
        :wait()
      if cur_sha then _write_sha(cur_sha) end
    end

    local undolevels = vim.bo[buf].undolevels
    vim.bo[buf].undolevels = -1
    vim.cmd.read({
      args = { fn.fnameescape(fpath_md) },
    ...
    callback = function(info)
      ...
      vim.system(
        ...
        vim.schedule_wrap(function(obj)
          if not api.nvim_buf_is_valid(info.buf) then return end
          if obj.code ~= 0 then
            vim.notify(
          ...
          if fn.executable('sha256sum') == 1 then
            vim.system({ 'sha256sum', fpath_ipynb }, {}, function(_obj) _write_sha(_obj.stdout) end)
          end
        end)
      )
    end,
...
local function setup(buf)
  ...
  if vim.g.loaded_jupytext ~= nil or fn.executable('jupytext') == 0 then return end
