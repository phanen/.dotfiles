...
---@see nvim_open_win()
---@class fterm_winopts_t : api.keyset.win_config
---@field col number?
---@field row number?
---@field width number?
---@field height number?
      ...
      on_new = {
        ...
        auto_close = {
          ...
          callback = function(term)
            au('WinLeave', {
              buffer = term.buf,
              group = ag('FtermAutoClose' .. term.buf, {}),
        ...
        auto_resize = {
          ...
          callback = function(term)
            au('VimResized', {
              buffer = term.buf,
              group = ag('FtermAutoResize' .. term.buf, {}),
...
local function normalize_fterm_termopts(termopts)
  ...
  normalized_termopts =
    vim.tbl_deep_extend('force', fterm_opts_default.termopts, normalized_termopts) --[[@as table]]
...
function fterm_t:new(cmd, opts)
  ...
  local win = api.nvim_open_win(buf, true, normalized_opts.winopts)
  local jobid = fn.termopen(cmd, normalized_opts.jobopts)
...
function fterm_t:del()
  ...
  if self.jobid then fn.jobstop(self.jobid) end
  if self:buf_is_valid() then api.nvim_buf_delete(self.buf, { force = true }) end
...
---@return boolean
function fterm_t:buf_is_valid() return self.buf and api.nvim_buf_is_valid(self.buf) or false end
...
function fterm_t:open()
  ...
  local tab = api.nvim_get_current_tabpage()
  self.win[tab] = api.nvim_open_win(self.buf, true, normalize_fterm_winopts(self.opts.winopts))
...
function fterm_t:close(tab)
  ...
  api.nvim_win_close(self.win[tab], true)
  self.win[tab] = nil
end
...
function fterm_t:update_win_size()
  ...
  api.nvim_win_set_config(
    self.win[api.nvim_get_current_tabpage()],
...
function fterm_t:right_shift()
  if
    api.nvim_get_mode().mode:find('^t')
    and self:win_is_visible()
    and api.nvim_get_current_win() == self.win[api.nvim_get_current_tabpage()]
  then
