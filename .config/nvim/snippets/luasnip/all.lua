...
local get_date = function() return os.date('%d-%m-%y') end
...
local get_comment_string = function()
  local unwrap_cstr = function(cstr)
    ...
    assert((left or right), {
      msg = string.format(
        'Invalid commentstring for %s! Read `:h commentstring` for help.',
        vim.bo.filetype
      ),
    })
...
local marks = {
  -- signature
  function() return fmt('<{}>', i(1, username)) end,
  -- signature_with_email
  function() return fmt('<{}{}>', { i(1, username), i(2, ' ' .. email) }) end,
  -- date_signature_with_email
  function()
    return fmt('<{}{}{}>', { i(1, get_date()), i(2, ', ' .. username), i(3, ' ' .. email) })
  end,
  -- date_signature
  function() return fmt('<{}{}>', { i(1, get_date()), i(2, ', ' .. username) }) end,
  -- date
  function() return fmt('<{}>', i(1, get_date())) end,
  -- empty
