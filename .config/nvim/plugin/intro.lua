...
local logo = vim.g.modern_ui and '😄' or 'test'
...
local lines = {
  ...
  {
    chunks = {
      { text = string.format('Copyright (c) 2024 - %s developers', logo), hl = 'NonText' },
    },
  },
}
...
for _, line in ipairs(lines) do
  ...
  for _, chunk in ipairs(line.chunks) do
    ...
    chunk.width = fn.strdisplaywidth(chunk.text)
...
api.nvim_buf_set_lines(
  ...
  -1,
  false,
  vim.tbl_map(function(line) return string.rep(' ', line.offset) .. line.text end, lines)
)

-- Apply highlight groups
local ns = api.nvim_create_namespace('NvimIntro')
...
local win = api.nvim_open_win(buf, false, win_config)
...
au({
  ...
  callback = function(info)
    ...
    api.nvim_del_augroup_by_id(info.group)
    return true
  end,
})
